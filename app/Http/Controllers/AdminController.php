...
class AdminController extends Controller
{
    ...
    public function add_video_process(Request $request) {
        ...
        $video_validator = Validator::make( $request->all(), array(
         ...
         $trailer_video = $request->hasFile('trailer_video')?$request->file('trailer_video'):null;
        ...
        if($validator->fails()) {
            ...
            $video->sub_category_id = $request->sub_category_id?$request->sub_category_id:0;
	...
	$stories = Stories::select('stories.id',
						'stories.name' , 
						'stories.picture',
						'stories.status',
						'stories.category_id',
        ...
        return view('admin.stories')->with('stories' , $stories)->withPage('stories')->with('sub_page','view-stories');
    }

    public function add_story() {
        $categories=Category::all();
        return view('admin.add-story')->with('page' ,'stories')->with('categories',$categories)->with('sub_page' ,'add-story');
    }
    ...
    public function edit_story($id) {
        ...
        return view('admin.edit-story')->with('story' , $story)->with('categories',$categories)->with('page' ,'stories')->with('sub_page' ,'edit-story');
    }

    public function add_story_process(Request $request) {
        ...
        if($validator->fails()) {
            ...
            if($request->id != '') {
                ...
                $message = 'Story cannot be created';
                if($request->hasFile('picture')) {
                    Helper::delete_picture($story->picture);
                }
            } else {
                $message = 'Story created successfully.';
                //Add New User
                $story = new Stories;
                $story->is_approved = DEFAULT_TRUE;
            ...
            if($request->hasFile('picture') && $request->file('picture')->isValid()) {
                $story->picture = Helper::normal_upload_picture($request->file('picture'));
            }
            ...
            if($story) {
                ...
                return back()->with('flash_error', 'Story updated successfully.');
            }
    ...
    public function approve_story(Request $request) {

        $story = Stories::find($request->id);

        $story->is_approved = $request->status;
        ...
        if($story->is_approved == DEFAULT_TRUE){

            $message = tr('admin_not_category_approve');
        } else {

            if(count($story->subCategory) > 0) {

                foreach($story->subCategory as $sub_category)
                {                
                    $sub_category->is_approved = $story->is_approved;
            ...
            if(count($story->adminVideo) > 0) {

                foreach($story->adminVideo as $video)
                {                
                    $video->is_approved = $story->is_approved;
    ...
    public function delete_story(Request $request) {
        ...
        $story = Stories::where('id' , $request->story_id)->first();

        if($story) {
            $story->delete();
            return back()->with('flash_success', 'Story deleted successfully');
        } else {
